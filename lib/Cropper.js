...
var Cropper = function (_Component) {
    ...
    function Cropper(props) {
        ...
        const {limitHeight} = props
        if(limitHeight){
            defaultStyles.line = null
            defaultStyles.lineN.cursor = null
            defaultStyles.lineE.cursor = null
            defaultStyles.lineS.cursor = null
            defaultStyles.lineW.cursor = null
        }else{
    ...
    _createClass(Cropper, [{
        ...
        value: function calcPosition(width, height, left, top, callback) {
            ...
            this.setState({
                ...
                toImgTop4Style: top,
                frameWidth4Style: limitHeight || width,
                frameHeight4Style: limitHeight || height,
            }, function () {
        ...
        key: 'crop',
        value: function crop(maxWidth, maxHeight) {
            ...
            var wishedWidth = maxWidth ? Math.min(maxWidth, realFrameWidth) : realFrameWidth;
            var wishedHeight = maxHeight ? Math.min(maxHeight, realFrameHeight) : realFrameHeight;

            var squeezeRatio = wishedWidth / realFrameWidth;
            var flattenRatio = wishedHeight / realFrameHeight;
            var scaleRatio = Math.min(squeezeRatio, flattenRatio);

            var realOriginX = originX * _scale;
            var realOriginY = originY * _scale;

            var finalWidth = realFrameWidth * scaleRatio;
            var finalHeight = realFrameHeight * scaleRatio;
